...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for idx, h_dim in enumerate(hidden_dims):
      self.params["W%d" % (idx+1)] = weight_scale * np.random.randn(layer_input_dim, h_dim)
      self.params["b%d" % (idx+1)] = np.zeros(h_dim)

      if self.use_batchnorm:
        self.params['gamma%d' % (idx+1)] = np.ones(h_dim)
        self.params['beta%d' % (idx+1)] = np.zeros(h_dim)
  ...
  def loss(self, X, y=None):
    ...
    for i in xrange(1, self.num_layers):
      if self.use_batchnorm:
        l_out, l_cache[i] = affine_bn_relu_forward(layer_input, self.params['W%d'%(i)], self.params['b%d'%(i)], \
                                                  self.params['gamma%d'%(i)], self.params['beta%d'%(i)], self.bn_params[i-1])
      
      else:
        l_out, l_cache[i] = affine_relu_forward(layer_input, self.params['W%d' % (i)], \
                                                self.params['b%d' % (i)])

      if self.use_dropout:
        l_out, dp_cache[i] = dropout_forward(l_out, self.dropout_param)

      layer_input = l_out

    scores, o_cache = affine_forward(layer_input, self.params['W%d' % (self.num_layers)],\
    ...
    dx, dw, db = affine_backward(dout, o_cache)
    ...
    for i in xrange(1, self.num_layers):
      ...
      if self.use_dropout:
        dx = dropout_backward(dx, dp_cache[l])

      if self.use_batchnorm:
        dx, dw, db, dgamma, dbeta = affine_bn_relu_backward(dx, l_cache[l])

      else:
        dx, dw, db = affine_relu_backward(dx, l_cache[l])
